...
def train_and_predict(cfg, train_X, train_y, test_X, valid_X=None, valid_y=None):
    if cfg["model"] == "ridge":
        if cfg["task"] == "multi":
            ...
            test_preds = 0
            valid_preds = 0
            for n, i in enumerate(range(0, 5 * d, d)):
                ind = all_row_indices[i : i + d]
                wandb_start(cfg)
                cfg["fold_n"] = f"{fold_n}_{n}"
                model = RidgeRegressor(cfg, train_X[ind], train_y[ind].todense())
                model.train()
            ...
            wandb_start(cfg)
            model = RidgeRegressor(cfg, train_X, train_y.todense())

    elif cfg["model"] == "kernelridgerbf":
        if cfg["task"] == "multi":
            all_row_indices = np.arange(train_X.shape[0])
            np.random.shuffle(all_row_indices)
            d = train_X.shape[0] // 5
            ...
            for n, i in enumerate(range(0, 5 * d, d)):
                ...
                wandb_start(cfg)
                cfg["fold_n"] = f"{fold_n}_{n}"
                model = KernelRidgeRBFRegressor(
                    cfg, train_X[ind].todense(), train_y[ind].todense()
                )
                model.train()
                test_preds += model.predict(test_X.todense())
                if valid_X is not None:
                    valid_preds += model.predict(valid_X.todense())
            if valid_X is None:
                return test_preds
            else:
                return test_preds, valid_preds
            ...
            wandb_start(cfg)
            model = KernelRidgeRBFRegressor(cfg, train_X.todense(), train_y.todense())
            model.train()
            test_preds = model.predict(test_X.todense())
            if valid_X is None:
                return test_preds
        ...
        wandb_start(cfg)
        model = LightGBMRegressor(cfg, train_X, train_y.todense())

    elif cfg["model"] == "xgboost":
        wandb_start(cfg)
        model = XGBoostRegressor(cfg, train_X, train_y.todense())

    elif cfg["model"] == "catboost":
        wandb_start(cfg)
        model = CatBoostRegressor(cfg, train_X, train_y.toarray())

    elif cfg["model"] == "mlp":
        model = MLPRegressor(cfg, train_X, train_y, valid_X=valid_X, valid_y=valid_y)

    elif cfg["model"] == "largemlp":
        model = MLPLargeRegressor(
...
def hold_out(cfg, train_X, train_y, valid_X, valid_y, test_X):
    ...
    wandb.log({"mse": mse, "corrscore": corr})
...
def one_fold_gkf(fold_n, gkf, groups, cfg, train_X, train_y, test_X):
    print(f"[fold_{fold_n}] start")
    seed_everything(cfg["general"]["seed"], workers=True)
    train_indices, valid_indices = list(gkf.split(train_X, train_y, groups=groups))[
        fold_n
    ]
    train_X_cv, train_y_cv = (
        train_X[train_indices],
...
def all_train(cfg, train_X, train_y, test_X):
    ...
    # train
    test_preds = train_and_predict(cfg, train_X, train_y, test_X)
...
def main():
    ...
    if cfg["day_std_inputs"]:
        if cfg["task"] == "cite":
            ...
            if cfg["data_type"] == "svd_important_laurent_nodrop_raw_counts":
                ...
                ss.fit(scipy.sparse.vstack([train_X_values_day2], format="csr"))
                train_X_values_day2 = ss.transform(train_X_values_day2)
            else:
                ss = StandardScaler(with_mean=False)
                ss.fit(
                    scipy.sparse.vstack(
            ...
            # assign
            if cfg["data_type"] == "svd_important_laurent_nodrop_raw_counts":
                train_X_values[train_metadata["day"] == 2] = train_X_values_day2
                test_X_values_day2 = None
            else:
                (
                    train_X_values[train_metadata["day"] == 2],
                    test_X_values[test_metadata["day"] == 2],
    ...
    if cfg["general"]["cv"] == "donor":
        ...
        final_test_preds = np.mean(test_pred_list, axis=0)
        """
        ...
        # train all data
        cfg["fold_n"] = "all"
        final_test_preds = all_train(cfg, train_X_values, train_y_values, test_X_values)
