    ...
    def __init__(self, input_size, output_size, cfg):
        ...
        if cfg["batchnorm"]:
            self.network1 = nn.Sequential(
                nn.utils.weight_norm(nn.Linear(input_size, 64), dim=None),
                nn.BatchNorm1d(64),
                nn.Mish(),
                nn.Dropout(cfg["dropout"]),
            )
            self.network2 = nn.Sequential(
                nn.utils.weight_norm(nn.Linear(64, 64), dim=None),
                nn.BatchNorm1d(64),
                nn.Mish(),
                nn.Dropout(cfg["dropout"]),
            )
            self.network3 = nn.Sequential(
                nn.utils.weight_norm(nn.Linear(64, 32), dim=None),
                nn.BatchNorm1d(32),
                nn.Mish(),
                nn.Dropout(cfg["dropout"]),
            )
            self.network4 = nn.Sequential(
                nn.utils.weight_norm(nn.Linear(32, 32), dim=None),
                nn.BatchNorm1d(32),
                nn.Mish(),
                nn.Dropout(cfg["dropout"]),
            )
        ...
        self.fc = nn.Linear(64 + 64 + 32 + 32, output_size)
