    ...
    def __init__(self, input_size, output_size, cfg):
        ...
        if cfg["batchnorm"]:
            ...
            self.network2 = nn.Sequential(
                nn.utils.weight_norm(nn.Linear(1024, 1024), dim=None),
                nn.BatchNorm1d(1024),
                nn.Mish(),
                nn.Dropout(cfg["dropout"]),
            )
            ...
            self.network4 = nn.Sequential(
                nn.utils.weight_norm(nn.Linear(512, 512), dim=None),
                nn.BatchNorm1d(512),
                nn.Mish(),
                nn.Dropout(cfg["dropout"]),
            )
        ...
        self.fc = nn.Linear(1024 + 1024 + 512 + 512, output_size)
...
class MLPLargeRegressorInference:
